...
export function Certifications({ certifications }: { certifications: Certification[] }) {
  ...
  return (
    <section id="certifications" className="py-16 sm:py-24 px-4 sm:px-6 lg:px-8" style={{ backgroundColor: 'var(--bg)' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {certifications.map((cert, i) => (
            ...
            >
              <div className="flex items-start justify-between gap-3 mb-4">
                ...
                {cert.credential_url && (
                  <a href={cert.credential_url} target="_blank" rel="noopener noreferrer"
                    className="p-1.5 rounded-lg transition-all hover:bg-[var(--accent-light)] hover:text-[var(--accent)] flex-shrink-0"
              ...
              <h3 className="text-sm sm:text-base font-bold mb-1 group-hover:text-[var(--accent)] transition-colors" style={{ color: 'var(--text)' }}>
                {cert.title}
              </h3>
              <p className="text-xs font-semibold mb-2" style={{ color: 'var(--text-muted)' }}>{cert.issuer}</p>

              {cert.credential_id && (
                <span className="tag text-[10px] mb-2 w-fit">ID: {cert.credential_id}</span>
              )}
              ...
              {cert.skills?.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-auto pt-3 border-t" style={{ borderColor: 'var(--border)' }}>
                  {cert.skills.map((s, si) => <span key={si} className="tag text-[10px]">{s}</span>)}
                </div>
              )}
            </motion.div>
          ))}

...
export function Certifications({ certifications }: CertificationsProps) {
  ...
  return (
    <section id="certifications" className="py-24 px-4 sm:px-6 lg:px-8 relative bg-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {certifications.map((cert, index) => (
            <motion.div
              ...
              className="p-6 rounded-2xl bg-slate-50 border border-slate-200 hover:border-blue-300 hover:shadow-lg hover:shadow-blue-50 transition-all flex flex-col justify-between group"
            >
              <div>
                ...
                {cert.credential_id && (
                  <div className="inline-block px-2.5 py-1 rounded bg-slate-100 text-[11px] font-mono text-slate-500 mb-3">
              ...
              {cert.skills && cert.skills.length > 0 && (
                <div className="pt-3 border-t border-slate-200 flex flex-wrap gap-1">
                  {cert.skills.map((skill, sIdx) => (
                    <span
                      key={sIdx}
                      className="px-2 py-0.5 rounded text-[10px] font-mono bg-white text-slate-500 border border-slate-200"
                    >
                      {skill}
                    </span>
                  ))}

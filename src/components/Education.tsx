...
export function Education({ education }: { education: EducationType[] }) {
  ...
  return (
    <section id="education" className="py-16 sm:py-24 px-4 sm:px-6 lg:px-8" style={{ backgroundColor: 'var(--bg-surface)' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          {education.map((edu, i) => (
            <motion.div
              ...
              className="card-theme p-5 sm:p-6 flex flex-col"
            >
              ...
              {edu.grade && (
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-mono mb-3 w-fit"
              ...
              {edu.activities?.length > 0 && (
                <div className="pt-3 border-t mt-auto" style={{ borderColor: 'var(--border)' }}>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {edu.activities.map((a, ai) => <span key={ai} className="tag">{a}</span>)}
                  </div>
                </div>
              )}
            </motion.div>
